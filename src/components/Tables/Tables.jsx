...
export default function Tables() {
  ...
  const deleteRegister = async (registerId) => {
    ...
    if (confirmDelete.isConfirmed) {
      try {
        const response = await axios.delete(`/register/${registerId}`, {
          headers: {
            Authorization: `Bearer ${token}` // Incluye el token en el encabezado
          }
        });
  ...
  return (
    <div className="contenedor font-Poppins ">
      <div className="slider w-full bg-[#000] relative shadow-2xl h-[90px] items-center flex justify-center">
        ...
        <NavLink to="/home">
          <div className="toRegisters flex  w-[90px] justify-center rounded-sm text-white mr-8 cursor-pointer hover:scale-105 transition-all">
            <img src={formulario} alt="" />
            <p className="text-lg  ml-1 transition-all">Form</p>
          </div>
        </NavLink>
